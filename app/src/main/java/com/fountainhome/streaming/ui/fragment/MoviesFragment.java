...
public class MoviesFragment extends Fragment {
    ...
    @Override public void onViewCreated(@NonNull View v, @Nullable Bundle s) {
        ...
        b.pageTitle.setText("Movies"); b.backBtn.setVisibility(View.GONE);
        adapter = new ContentAdapter(item -> {
            Intent i = new Intent(getContext(), WatchActivity.class);
            i.putExtra("type", "movie"); i.putExtra("id", item.id); startActivity(i);
        });
    ...
    private void load(int page) {
        ...
        switch (tab) {
            case "trending": call = TMDBClient.get().getTopRatedMovies(page); break;
            case "popular":  call = TMDBClient.get().getPopularMovies(page); break;
            default:         call = TMDBClient.get().getNowPlaying(page); break;
        }
        call.enqueue(new Callback<Models.PagedResponse<Models.MovieResult>>() {
            public void onResponse(@NonNull Call<Models.PagedResponse<Models.MovieResult>> c,
                ...
                if (r.isSuccessful() && r.body() != null && r.body().results != null) {
                    List<Models.MovieResult> results = r.body().results;
                    if (results.isEmpty()) { hasMore = false; return; }
                    currentPage = page;
                    for (Models.MovieResult m : results) {
